...
struct TreeDumpWalker : public ExpressionVisitor {
  ...
  antlrcpp::Any visitExpr(ExpressionParser::ExprContext *ctx) override {
    ...
    if (ctx->children.size() == 2) {
      outs() << Tab << "neg\n";
      res = -std::any_cast<int>(visitExpr(ctx->expr()[0]));
    }
    if (ctx->children.size() == 3) {
      // '{' expr '}'
      if (ctx->children[0]->getText().at(0) == '{') {
        outs() << Tab << "{ }\n";
        res = std::any_cast<int>(visit(ctx->children[1]));
      } else {
        // expr ( '*' | '/') expr
        // expr ( '+' | '-') expr
        int lhs = std::any_cast<int>(visit(ctx->children[0]));
        outs() << Tab << ctx->children[1]->getText() << "\n";
        int rhs = std::any_cast<int>(visit(ctx->children[2]));
        switch (ctx->children[1]->getText().at(0)) {
        case '*':
          res = lhs * rhs;
          break;
...
struct TreeWalker : public ExpressionVisitor {
  antlrcpp::Any visitExpr(ExpressionParser::ExprContext *ctx) override {
    ...
    // '-' expr
    if (ctx->children.size() == 2) {
      return -std::any_cast<int>(visitExpr(ctx->expr()[0]));
    }
    // '{' expr '}'
    if (ctx->children[0]->getText().at(0) == '{') {
      return visit(ctx->children[1]);
...
int main(int argc, const char *argv[]) {
  ...
  builder.CreateRet(std::any_cast<Value *>(walker.visitExpr(parser.expr())));
